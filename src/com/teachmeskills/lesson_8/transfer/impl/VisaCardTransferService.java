...
public class VisaCardTransferService implements CardTransferService {
    ...
    public Check transferFromCardToCard(BaseCard cardFrom, BaseCard cardTo, double transferAmount) {
        if(cardFrom.checkCardLimitTransfer(transferAmount)){
            ...
            return new Check(transferAmount, new Date(), cardFrom.cardNumber, cardTo.cardNumber, "Limits were exceeded. Transfer limit for this card = " + Constants.VISA_CARD_LIMIT);
        }
    }
    ...
    public Check transferFromCardToAccount(BaseCard cardFrom, Account accountTo, double transferAmount) {
        if (cardFrom.checkCardLimitTransfer(transferAmount)) {
            ...
            return new Check(transferAmount, new Date(), cardFrom.cardNumber, accountTo.getNumber(), "Limits were exceeded. Transfer limit for this card = " + Constants.VISA_CARD_LIMIT);
        }
    }
}
